...
public class StudentServiceTests {
	...
	@Test
	public void addQuestionToStudentWithoutCreatingNewQuestion_test() {
		Student student = new Student();
		student.setId((long) 1);
		int beforeQuestionNum = student.getStudentQuestions().size();
		
		Question tempQuestion = new Question();
		tempQuestion.setId((long)1);
		studentService.addQuestionToStudentWithoutCreatingNewQuestion(tempQuestion, student);
		int afterQuestionNum = student.getStudentQuestions().size();
		
		assertEquals(beforeQuestionNum+1, afterQuestionNum);
	}
